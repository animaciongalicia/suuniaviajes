...
const navigation = [
  { name: "Inicio", href: "/" },
  { name: "Experiencias", href: "/receptivo-galicia" },
    ...
    <header className="fixed top-0 z-50 w-full border-b border-gray-100 bg-white/95 backdrop-blur-md">
      <nav className="mx-auto flex max-w-7xl items-center justify-between px-6 py-4 lg:px-8">
        <Link href="/" className="flex items-center gap-2">
          <span className="text-2xl font-bold tracking-tight text-atlantic-800">
            SUUNIA
          </span>
        ...
        <div className="hidden items-center gap-6 lg:flex">
          {navigation.map((item) => (
            <Link
              key={item.name}
              href={item.href}
              className="text-sm font-medium text-gray-600 transition-colors hover:text-atlantic-700"
            >
              {item.name}
            </Link>
          ))}
        ...
        <div className="border-t border-gray-100 bg-white px-6 pb-6 pt-4 lg:hidden">
          {navigation.map((item) => (
            <Link
              ...
              className="block py-3 text-sm font-medium text-gray-600 transition-colors hover:text-atlantic-700"
            >
              {item.name}
            </Link>
          ))}
